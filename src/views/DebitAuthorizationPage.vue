<template>
  <div>
    <div class="flex flex-col items-center justify-center px-4 py-8 mx-auto space-y-4">
      <div class="font-bold text-lg">
        AUTORIZAÇÃO DE DÉBITO EM CARTÃO DE CRÉDITO
      </div>

      <div class="space-y-9">
        <Form @submit="onSubmit" v-slot="{ setFieldValue, values }" :initial-values="formValues">
          <div class="grid grid-cols-4 grid-rows-4 gap-6">
            <Select label="CARTÃO DE CRÉDITO" class="col-span-2" :options="options" @selectValue="
              (e) => {
                cartao = e.value;
              }
            " />

            <button class="bg-blue-700 hover:bg-blue-800 text-white font-medium px-5 py-4">
              Gerar documento
            </button>
            ...
            <TextInput nameValue="numberCard" rulesValue="required|digitsCard" label="NÚMERO DO CARTÃO"
              maska="####-####-####-####" />
            <TextInput nameValue="nameCard" rulesValue="required|alpha_spaces" label="NOME TITULAR" />
            <TextInput nameValue="validCardDate" rulesValue="required|validCard" label="VALIDADE DO CARTÃO"
              placeholder="MM/AA" maska="##/##" />

            <TextInput nameValue="codeCard" rulesValue="required" label="CVV/CVC" maska="####" />

            <TextInput nameValue="cpfNumber" rulesValue="required|cpfNumber" class="col-span-2" label="CPF Nº"
              maska="###.###.###-##" />
            <TextInput nameValue="rneNumber" class="col-span-2" label="RG/RNM" />

            <TextInput nameValue="phoneNumber" rulesValue="required|phone" class="col-span-2" label="TELEFONE"
              maska="(##) #####-####" />
            <TextInput nameValue="transactionValue" rulesValue="required" label="VALOR DA TRANSAÇÃO" maska="#*.##" />

            <TextInput nameValue="dataTransacao" rulesValue="required|invaliDate" label="DATA DA TRANSAÇÃO"
              typeValue="date" />

            <TextInput nameValue="valorPrimeiraParcela" rulesValue="required|minValue:transactionValue"
              class="col-span-2" label="VALOR DE PRIMEIRA PARCELA" maska="#*.##" />
            <TextInput nameValue="numberParcela" maska="##" rulesValue="required" label="Nº DE PARCELAS" @input="
              setFieldValue(
                'restosParcelas',
                calculate(
            ...
            " />
            <TextInput nameValue="restosParcelas" rulesValue="required" label="RESTOS DE PARCELAS" />
          </div>
        </Form>
        ...
        <div class="grid grid-cols-2 grid-rows-1 gap-4">
          <div>
            <Form @submit="addPassageiro">
              <div class="flex">
                <TextInput nameValue="inputPassageiros" rulesValue="required" class="basis-4/5" label="Passageiros" />
                <button class="basis-1/5 bg-blue-700 hover:bg-blue-800 cursor-pointer text-center text-white">
                  Adicionar
                </button>
              </div>
              <ul class="py-5">
                  ...
                  <li v-for="(passageiro, index) in passageiros" :key="index">
                    <div class="flex items-center justify-between px-3">
                      ...
                      <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 cursor-pointer"
                        @click="deletePassageiro(index)" fill="none" viewBox="0 0 24 24" stroke="currentColor"
                        stroke-width="2">
          ...
          <div>
            <Form @submit="addDestinasePagamento">
              <div class="flex">
                <TextInput nameValue="inputdestinasePagamento" rulesValue="required" class="basis-4/5"
                ...
                <TransitionGroup>
                  <li v-for="(item, index) in destinasePagamento" :key="index">
                    <div class="flex items-center justify-between px-3">
                      ...
                      <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 cursor-pointer"
                        @click="deletePagamento(index)" fill="none" viewBox="0 0 24 24" stroke="currentColor"
                        stroke-width="2">
                        <path stroke-linecap="round" stroke-linejoin="round"
                          d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                      </svg>
