...
const register = () => {
  auth.loading = true;
  const datetime = new Date(auth.user.birthday);
  const birthday = `${datetime.getFullYear()}-${(
    '0' +
    (datetime.getMonth() + 1)
  ...
  auth
    ...
    .then(({ data }) => {
      if (data.isRegister) {
        alertStore.showMsg({
          message:
            'Se ha enviado un correo electrónico de confirmación. Por favor, regístrese e inicie sesión.',
          backgrColor: 'blue',
          textColor: 'blue',
        });
        auth.loading = false;
