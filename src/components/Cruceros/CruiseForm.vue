<template>
    <div>
      <div class="mt-5 flex flex-col space-y-1">
        ...
        <div class="px-4 mb-3">
          <Dropddown label="Hóspedes">
            <template v-slot:selected>
              <span>{{lodging}} Hospede - {{adults}} Adultos</span>
            </template>
            <template v-slot:dropdown>
              <div class="flex items-center space-x-10 px-4">
                <ManageItems label="Hospede" v-model="lodging" @takeOff="takeLodging" @addUp="addLodging"/>
                <ManageItems label="Adultos" v-model="adults" @takeOff="takeAdult" @addUp="addAdult"/>
              </div>
            </template>
          </Dropddown>
        </div>
        ...
        <div class="px-4">
          <AutoComplete label="Destino" :value="origem.string" @input="
            (e) => {
              origem.string = e;
            }
          " @select="
        ...
        <div class="grid grid-cols-2 grid-rows-1 pt-4 px-4">
          <DateInput v-model="flightDates.departureDate" label="Data de Embarque" />
          <DateInput v-model="flightDates.returnDate" :dateMin="notBeforeDate()" />
        </div>

        <div class="px-4 pt-3">
          <AutoComplete label="Portos de Embarque" :value="origem.string" @input="
            (e) => {
              origem.string = e;
            }
          " @select="
