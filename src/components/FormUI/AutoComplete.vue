<template>
  <div class="relative">
    <div class="relative">
      ...
      <input type="text" ref="input" :value="value" :class="inputClassList" @input="onInput($event.target.value)"
        @keydown="onKeydown" @blur="onBlur" :placeholder="placeholder" />
    ...
    <div v-show="options.length" class="absolute right-0 mt-2 w-full rounded-md shadow-lg z-50 overflow-y-scroll"
      style="max-height: 200px">
      <ul class="py-1 rounded-md bg-white shadow-xs">
        <li v-for="(opt, index) in options" :key="index" tabindex="0" @click="onSelect()"
          @mouseover="setArrowCounter(index)"
          class="autocomplete-item block px-4 py-2 text-sm leading-5 text-gray-700 cursor-pointer"
          :class="{ 'bg-gray-200': arrowCounter === index }" :ref="el => { optionsList[index] = el }">
          <div v-if="opt.nameCity" class="flex items-center">
            ...
            <span class="font-normal" v-html="opt['nameCity_highligthed'] || opt['nameCity']" />
          </div>
          ...
          <div v-else class="flex items-center">
            ...
            <span class="font-normal" v-html="opt['nameAirport_highligthed'] || opt['nameAirport']" />
          </div>
        </li>
      </ul>
    </div>
...
const highlightOptions = () => {
  ...
  options.value.forEach((element) => {
    if (element.nameCity) {
      element['nameCity_highligthed'] = element["nameCity"].replace(query, '<span class="font-bold">$&</span>')
    } else {
      element['nameAirport_highligthed'] = element['nameAirport'].replace(query, '<span class="font-bold">$&</span>')
    }
  })
}
