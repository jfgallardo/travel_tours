<template>
  <div>
    <Carousel @next="nextSlide" @prev="prevSlide">
      ...
      <CarouselSlider v-for="slide in itemsToDisplay" :key="slide.city" :slide="slide">
        <template v-slot:image>
          <img v-bind:src="slide.src" class="w-full h-64  object-cover" @mousedown="nextSlide" />
        </template>
        <template v-slot:footer>
          <span class="text-xs text-gray-600">Viaje + Alojamiento + Carro + Tours Guide</span><br />
          <span class="text-sm font-semibold">${{ slide.price }} | {{ slide.people }} Personas |
