  ...
  <div>
    <div>
      <form class="grid grid-cols-3 gap-6">
        <Select label="PAYMENT TYPES" :selected="paymentSelect" :options="paymentTypes" class="col-span-2" @selectValue="
          (e) => {
            paymentSelect = e;
          }
        " />

        <Select label="CARTÃO DE CRÉDITO" :selected="pdfStore.creditCard.cartao" v-if="paymentSelect.value === 'C'"
          :options="typeCards" @selectValue="
            (e) => {
              cartao = e;
            }
          " />

        <DateInput label="DATA DA TRANSAÇÃO" name="dateTransaction" v-model="dateTransaction" />

        <TextInput name="numberCard" label="NÚMERO DO CARTÃO" maska="####-####-####-####"
          v-if="paymentSelect.value === 'C'" />

        <TextInput name="nameCard" label="NOME TITULAR" v-if="paymentSelect.value === 'C'" />

        <TextInput name="validCardDate" label="VALIDADE DO CARTÃO" placeholder="MM/AA" maska="##/##"
          v-if="paymentSelect.value === 'C'" />

        <TextInput name="securityCode" label="CÓDIGO DE SEGURANÇA" :maska="cartao.value === 'AMEX' ? '####' : '###'"
          v-if="paymentSelect.value === 'C'" />

        <TextInput name="transaction_value" label="VALOR DA TRANSAÇÃO" maska="#*.##" v-model="transactionValue" />

        <TextInput name="route" label="ITINERARIO" class="col-span-2"
          placeholder="Adicionar separado por bar. Exemplo: BSB/PTY/HAV" />

        <TextInput v-if="paymentSelect.value === 'C'" name="rate" label="TARIFA" maska="#*.##" v-model="tariff" />

        <TextInput v-if="paymentSelect.value === 'C'" name="rav" maska="#*.##" label="RAV/DU" v-model="rav" />

        <TextInput v-if="paymentSelect.value === 'C'" name="rc" label="RC" maska="#*.##" v-model="rc" />

        <TextInput v-if="paymentSelect.value === 'C'" name="taxa_embarque" maska="#*.##" label="TAXA DE EMBARQUE"
          v-model="taxa_embarque" />

        <TextInput v-if="paymentSelect.value === 'C'" name="number_plots" maska="##" label="Nº DE PARCELAS"
          v-model="numberParcels" />

        <CheckInput class="place-self-end" v-model="proportionalValue" label="Gerar parcelas proporcionais"
          v-if="paymentSelect.value === 'C'" />
        ...
        <h6 v-if="paymentSelect.value === 'C'" class="text-sm font-medium leading-tight text-gray-800">
          Primera parcela
          <span
      ...
      <template v-slot:body>
        <div class="bg-white px-4 pb-4 sm:p-6 sm:pb-4">
          ...
          <div class="float-right">
            <button type="button" @click="saveCC"
            ...
            <button type="button" @click="notSaveCC"
...
watch(
  ...
  ([newParcels, newProportionalValue]) => {
    ...
    if (newProportionalValue) {
      const proportional = (parseFloat(transactionValue.value) / parseFloat(numberParcels.value)).toFixed(2)
